...
async function walletSync() {
  if (process.env.TESTNET === 'true') {
    throw new Error('No testnet API available.');
  }
  ...
  try {
    const response = await axios.get(`https://dogechain.info/api/v1/address/unspent/${wallet.address}`);
...
function inscribe(wallet, address, contentType, data) {
	...
	inscription.chunks.push(numberToChunk(112))
    inscription.chunks.push(bufferToChunk('cdrc-20:mint:cdrc=1000'))
